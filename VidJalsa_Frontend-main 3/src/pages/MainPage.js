...
const MainPage = () => {
    ...
    useEffect(() => {
        const fetchTrendingTopics = async () => {
            try {
                const response = await fetch('http://127.0.0.1:7000/api/v1/trending');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://127.0.0.1:7000/api/v1/videos_preview', {
                method: 'POST',
                headers: {
            ...
            navigate('/preview', { state: { videosInfo: data, topic: topic } });
            setIsLoading(false);
        } catch (error) {
                ...
                <div className={styles.inputContainer}>
                    ...
                    <form onSubmit={handleSubmit} className={styles.formContainer}>
                        <input
                            id="topicInput"
                            type="text"
                            className={topic ? 'withButton leftAlignText' : ''}
                            value={topic}
